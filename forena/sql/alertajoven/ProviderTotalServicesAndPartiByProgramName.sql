SELECT 
    subTotalActivities.providerName, 
    totalServices,
    subTotalPartici.totalParticipants 
FROM
    (
        SELECT 
            ifnull(services_count, 0) as 'totalServices', 
            agency.field_agency_name_value as 'providerName', 
            agency.entity_id as 'providerId1'   
        FROM
            (
                SELECT
...
--END
                    ...
                    order by provider.field_agency_name_value
            ) sub

        JOIN bitnami_drupal7.field_data_field_agency_name agency ON agency.entity_id = sub.provider_id
        order by agency.field_agency_name_value) 
    subTotalActivities,

(SELECT 
    ifnull(UniqueParticipants,0) as 'totalParticipants', 
    agency.field_agency_name_value, 
    agency.entity_id as 'providerId2' 
FROM
(SELECT
...
JOIN bitnami_drupal7.field_data_field_agency_name agency ON agency.entity_id = sub2.provider_id
